...
def main():
    ...
    parser.add_argument("--output-clusters", "-oc",
                        help="nuber of output clusters: 0 ~ output word vectors; 1 ~ nubmber k-means clusters", default=0, type=int)
    ...
    if args.output_clusters > 0:
        args.output_file = args.text_data_file.replace(".txt", ".cluster.txt")
    else:
        if args.binary == 0:
            args.output_file = args.text_data_file.replace(".txt", ".embedding.txt")
        elif args.binary == 1:
            args.output_file = args.text_data_file.replace(".txt", ".embedding.bin")
        else:
            args.output_file = args.text_data_file.replace(".txt", ".embedding.txt")
